...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_msg = (
        ...
        f"أهلاً بك يا {user.first_name} في بوت حاشية أزهري لتنظيم المذاكرة والمهام! 🦅\n\n"
...
def get_categories_keyboard(branch):
    keyboard = []
    for cat in SUBJECTS_DATA[branch].keys():
        keyboard.append([InlineKeyboardButton(cat, callback_data=f"show_cat_{cat}")])
...
def get_subjects_keyboard(user_id, branch, category):
    selected = user_selected_subjects.get(user_id, set())
    subjects = SUBJECTS_DATA[branch][category]
    ...
    for subj in subjects:
        ...
        row.append(InlineKeyboardButton(btn_text, callback_data=f"toggle_subj_{category}_{subj}"))
        if len(row) == 2:
            keyboard.append(row)
            row = []
    if row:
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_branch":
        ...
        await query.edit_message_text(
            ...
            reply_markup=get_subjects_keyboard(user_id, branch, cat_name),
            parse_mode="Markdown"
        )
